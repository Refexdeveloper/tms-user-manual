/**
 * Airline logo helper for Travel Booking flight results.
 * Resolves IATA codes → logo URL (Kiwi / AVS CDN by code).
 * Name map covers common carriers; unknown codes still try CDN by IATA.
 * Reference catalog: https://1000logos.net/airline/
 */

const NAME_TO_CODE = {
  indigo: '6E',
  'indi go': '6E',
  'air india': 'AI',
  'airindia': 'AI',
  'air india express': 'IX',
  vistara: 'UK',
  spicejet: 'SG',
  'spice jet': 'SG',
  goair: 'G8',
  'go air': 'G8',
  gofirst: 'G8',
  'go first': 'G8',
  akasa: 'QP',
  'akasa air': 'QP',
  allianceair: '9I',
  'alliance air': '9I',
  emirates: 'EK',
  etihad: 'EY',
  'etihad airways': 'EY',
  qatar: 'QR',
  'qatar airways': 'QR',
  'singapore airlines': 'SQ',
  singapore: 'SQ',
  'thai airways': 'TG',
  thai: 'TG',
  'cathay pacific': 'CX',
  cathay: 'CX',
  'british airways': 'BA',
  lufthansa: 'LH',
  klm: 'KL',
  'air france': 'AF',
  turkish: 'TK',
  'turkish airlines': 'TK',
  gulf: 'GF',
  'gulf air': 'GF',
  'saudi arabian': 'SV',
  saudia: 'SV',
  oman: 'WY',
  'oman air': 'WY',
  'malaysia airlines': 'MH',
  malaysia: 'MH',
  batik: 'ID',
  scoot: 'TR',
  flydubai: 'FZ',
  indigoairlines: '6E',
  jetairways: '9W',
  'jet airways': '9W',
  indigoaviation: '6E',
}

export function resolveAirlineCode(airlineCode, airlineName) {
  const raw = String(airlineCode || '').trim().toUpperCase()
  if (/^[A-Z0-9]{2}$/.test(raw)) return raw
  // flight numbers sometimes include code prefix e.g. 6E-234
  const fromNum = raw.match(/^([A-Z0-9]{2})/)
  if (fromNum) return fromNum[1]

  const name = String(airlineName || '')
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  if (!name) return ''
  if (NAME_TO_CODE[name]) return NAME_TO_CODE[name]
  for (const [key, code] of Object.entries(NAME_TO_CODE)) {
    if (name.includes(key) || key.includes(name)) return code
  }
  return ''
}

/** Prefer Google Flights + Kiwi + AVS logos by IATA */
export function airlineLogoUrls(code) {
  if (!code) return []
  const c = code.toUpperCase()
  return [
    `https://www.gstatic.com/flights/airline_logos/70px/${c}.png`,
    `https://images.kiwi.com/airlines/64/${c}.png`,
    `https://pics.avs.io/80/80/${c}@2x.png`,
    `https://pics.avs.io/64/64/${c}.png`,
  ]
}

export function airlineLogoUrl(airlineCode, airlineName) {
  const code = resolveAirlineCode(airlineCode, airlineName)
  return code ? airlineLogoUrls(code)[0] : ''
}
